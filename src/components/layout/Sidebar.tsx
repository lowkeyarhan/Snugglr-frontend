...
export default function Sidebar({ collapsed = false }: { collapsed?: boolean }) {
  ...
  const userHandle = user?.username ? `@${user.username}` : "@guest";
  ...
  return (
    <aside
      className={`${collapsed ? "w-20" : "w-65"} hidden lg:flex flex-col border-r border-gray-100 dark:border-gray-800 h-screen bg-white/80 dark:bg-surface-dark/80 backdrop-blur-md flex-shrink-0 transition-all duration-300`}
    >
      <div className={`flex items-center gap-3 ${collapsed ? "px-4 py-8 justify-center" : "px-6 py-8"}`}>
        {!collapsed && (
      ...
      <nav className={`flex flex-col gap-2 ${collapsed ? "px-1" : "px-2"} pb-4 pt-2 flex-1`}>
        {items.map(({ icon, label, path, filled, badge }) => {
          ...
          return (
            <button
              ...
              className={`flex items-center ${collapsed ? "justify-center px-3" : "gap-4 px-4"} py-3 rounded-2xl transition-all duration-200 text-slate-900 dark:text-slate-100 cursor-pointer group ${isActive
                ? "bg-black/10 dark:bg-slate-900/50"
                : "text-gray-500 hover:bg-gray-50 dark:text-gray-400 dark:hover:bg-white/5"
                }`}
            >
              <span className="relative">
                <span className={`material-symbols-outlined text-[28px] group-hover:scale-105 transition-transform ${isActive || filled ? "fill-current" : ""}`}>
                  {icon}
                </span>
                {badge && (
